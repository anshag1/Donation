...
- **Self-service password reset**: `POST /auth/forgot-password` → `POST /auth/reset-password` — a single-use, hashed, 1-hour-expiry token emailed via Resend (never logged or returned in the API response — see the security-review-caught invite-token-logging finding below for why that distinction matters). `/forgot-password` always returns the same response whether or not the email is registered, so it can't be used to enumerate admin accounts. A successful reset also clears any active account lockout (proving control of the registered email is itself a strong identity check, and it's the only self-service way to recover from a lockout today — before this, an admin locked out or with a forgotten password had no way back in except a super_admin or developer editing the database directly, which is how a lockout during this project's own development was resolved).
